export class GameConstants {
	...
	private static mobHpVal: number[] = [0, 15, 20, 25, 35, 50, 65, 80, 95, 110, 125, 150, 175, 200, 225, 250, 275, 300, 325, 350,
	...
	public static JAIL = 180000002; MAX_BUFFSTAT = 4;
	public static blockedSkills = [4341003];
	public static RESERVED:string[] = ["WhiteJS"];
	public static stats:string[] = ["tuc", "reqLevel", "reqJob", "reqSTR", "reqDEX", "reqINT", "reqLUK", "reqPOP", "cash", "cursed", "success", "setItemID", "equipTradeBlock", "durability", "randOption", "randStat", "masterLevel", "reqSkillLevel", "elemDefault", "incRMAS", "incRMAF", "incRMAI", "incRMAL", "canLevel", "skill", "charmEXP", "pickupAll", "pickupItem", "sweepForDrop", "longRange", "consumeHP", "consumeMP", "noRevive"];
	public static hyperTele = [310000000, 220000000, 100000000, 250000000, 240000000, 104000000, 103000000, 102000000, 101000000, 120000000, 260000000, 200000000, 230000000];

	public static getExpNeededForLevel(level:number):number {
			if (level < 0 || level >= this.exp.length) {
					return Number.MAX_SAFE_INTEGER;
			}
			return this.exp[level];
	}

	public static getGuildExpNeededForLevel(level:number) :number{
			if (level < 0 || level >= this.guildexp.length) {
					return Number.MAX_SAFE_INTEGER;
			}
			return this.guildexp[level];
	}

	public static getPVPExpNeededForLevel(level:number):number {
			if (level < 0 || level >= this.pvpExp.length) {
					return Number.MAX_SAFE_INTEGER;
			}
			return this.pvpExp[level];
	}

	public static getClosenessNeededForLevel(level:number) {
			return this.closeness[level - 1];
	}

	public static getIntForCloseness(exp:number) {
			for (let i = 0; i < this.closeness.length; ++i) {
					if (this.closeness[i] > exp) {
							return i;
					}
			}
			return 1;
	...
	public static getMountExpNeededForLevel(level:number):number {
			return this.mountexp[level - 1];
	}

	public static getTraitExpNeededForLevel(level:number):number {
			if (level < 0 || level >= this.cumulativeTraitExp.length) {
					return Number.MAX_SAFE_INTEGER;
			}
			return this.cumulativeTraitExp[level];
	}

	public static getSetExpNeededForLevel(level:number):number {
			if (level < 0 || level >= this.setScore.length) {
					return Number.MAX_SAFE_INTEGER;
			}
			return this.setScore[level];
	}

	public static getMonsterHP(level:number):number {
			if (level < 0 || level >= this.mobHpVal.length) {
					return Number.MAX_SAFE_INTEGER;
			}
			return this.mobHpVal[level];
	}

	public static getBookLevel(level:number):number {
			return ((5 * level) * (level + 1));
	}

	public static getTimelessRequiredEXP(level:number):number {
			return 70 + (level * 10);
	}

	public static getReverseRequiredEXP(level:number):number {
			return 60 + (level * 5);
	}

	public static isHarvesting(itemId:number):boolean {
			return itemId >= 1500000 && itemId < 1520000;
	}

	public static maxViewRangeSq():number {
			return 1000000; // 1024 * 768
	}

	public static maxViewRangeSq_Half():number {
			return 500000; // 800 * 800
	}

	public static isJobFamily(baseJob:number, currentJob:number) :boolean{
			return currentJob >= baseJob && currentJob / 100 == baseJob / 100;
	}

	public static isKOC(job:number) :boolean{
			return job >= 1000 && job < 2000;
	}

	public static isEvan(job:number) :boolean{
			return job == 2001 || (job >= 2200 && job <= 2218);
	}

	public static isMercedes(job:number) :boolean{
			return job == 2002 || (job >= 2300 && job <= 2312);
	}

	public static isDemon(job:number) :boolean{
			return job == 3001 || (job >= 3100 && job <= 3112);
	}

	public static isAran(job:number) :boolean{
			return job >= 2000 && job <= 2112 && job != 2001 && job != 2002;
	}

	public static isResist(job:number) :boolean{
			return job >= 3000 && job <= 3512;
	}

	public static isAdventurer(job:number) :boolean{
			return job >= 0 && job < 1000;
	}

	public static isCannon(job:number) :boolean{
			return job == 1 || job == 501 || (job >= 530 && job <= 532);
	}

	public static isRecoveryIncSkill(id:number) :boolean{
			switch (id) {
					case 1110000:
					case 2000000:
					case 1210000:
	...
	public static isLinkedAranSkill(id:number) :boolean{
			return this.getLinkedAranSkill(id) != id;
	}

	public static getLinkedAranSkill(id:number):number {
			switch (id) {
					case 21110007:
					case 21110008:
							return 21110002;
	...
	public static  isForceIncrease(skillid:number) :boolean{
			switch (skillid) {
					case 31000004:
					case 31001006:
					case 31001007:
	...
	// public static getBOF_ForJob(job:number):number {
	// 		return this.PlayerStats.getSkillByJob(12, job);
	// }

	// public static getEmpress_ForJob(job:number):number {
	// 		return this.PlayerStats.getSkillByJob(73, job);
	// }

	public static isElementAmp_Skill(skill:number) :boolean{
			switch (skill) {
					case 2110001:
					case 2210001:
					case 12110001:
	...
	public static getMPEaterForJob(job:number):number {
			switch (job) {
					case 210:
					case 211:
					case 212:
	...
	public static getJobShortValue(job:number):number {
			if (job >= 1000) {
					job -= (job / 1000) * 1000;
			}
			job /= 100;
...
// 	public static isPyramidSkill(skill:number) :boolean{
// 			return isBeginnerJob(skill / 10000) && skill % 10000 == 1020;
// 	}

// 	public static isInflationSkill(skill:number) :boolean{
// 			return isBeginnerJob(skill / 10000) && skill % 10000 == 1092;
// 	}

// 	public static isMulungSkill(skill:number) :boolean{
// 			return isBeginnerJob(skill / 10000) && (skill % 10000 == 1009 || skill % 10000 == 1010 || skill % 10000 == 1011);
// 	}

// 	public static isIceKnightSkill(skill:number) :boolean{
...
// 	public static isThrowingStar(itemId:number) :boolean{
// 			return itemId / 10000 == 207;
// 	}

// 	public static isBullet(itemId:number) :boolean{
// 			return itemId / 10000 == 233;
// 	}

// 	public static isRechargable(itemId:number) :boolean{
// 			return isThrowingStar(itemId) || isBullet(itemId);
// 	}

// 	public static isOverall(itemId:number) :boolean{
// 			return itemId / 10000 == 105;
// 	}

// 	public static isPet(itemId:number) :boolean{
// 			return itemId / 10000 == 500;
// 	}

// 	public static isArrowForCrossBow(itemId:number) :boolean{
// 			return itemId >= 2061000 && itemId < 2062000;
// 	}

// 	public static isArrowForBow(itemId:number) :boolean{
// 			return itemId >= 2060000 && itemId < 2061000;
// 	}

// 	public static isMagicWeapon(itemId:number) :boolean{
// 			const s = itemId / 10000;
// 			return s == 137 || s == 138;
// 	}

// 	public static isWeapon(itemId:number) :boolean{
// 			return itemId >= 1300000 && itemId < 1500000;
// 	}

// 	// public static MapleInventoryType getInventoryType(itemId:number) {
...
// 	// public static MapleWeaponType getWeaponType(itemId:number) {
...
// 	public static isShield(itemId:number) :boolean{
// 			const cat = itemId / 10000;
// 			cat = cat % 100;
// 			return cat == 9;
// 	}

// 	public static isEquip(itemId:number) :boolean{
// 			return itemId / 1000000 == 1;
// 	}

// 	public static isCleanSlate(itemId:number) :boolean{
// 			return itemId / 100 == 20490;
// 	}

// 	public static isAccessoryScroll(itemId:number) :boolean{
// 			return itemId / 100 == 20492;
// 	}

// 	public static isChaosScroll(itemId:number) :boolean{
...
// 	public static getChaosNumber(itemId:number):number {
// 			return itemId == 2049116 ? 10 : 5;
// 	}

// 	public static isEquipScroll(scrollId:number) :boolean{
// 			return scrollId / 100 == 20493;
// 	}

// 	public static isPotentialScroll(scrollId:number) :boolean{
// 			return scrollId / 100 == 20494 || scrollId == 5534000;
// 	}

// 	public static isSpecialScroll(scrollId:number) :boolean{
// 			switch (scrollId) {
...
// 	public static isTwoHanded(itemId:number) :boolean{
...
// 	public static isTownScroll(id:number) :boolean{
// 			return id >= 2030000 && id < 2040000;
// 	}

// 	public static isUpgradeScroll(id:number) :boolean{
// 			return id >= 2040000 && id < 2050000;
// 	}

// 	public static isGun(id:number) :boolean{
// 			return id >= 1492000 && id < 1500000;
// 	}

// 	public static isUse(id:number) :boolean{
// 			return id >= 2000000 && id < 3000000;
// 	}

// 	public static isSummonSack(id:number) :boolean{
// 			return id / 10000 == 210;
// 	}

// 	public static isMonsterCard(id:number) :boolean{
// 			return id / 10000 == 238;
// 	}

// 	public static isSpecialCard(id:number) :boolean{
// 			return id / 1000 >= 2388;
// 	}

// 	public static getCardShortId(id:number):number {
// 			return id % 10000;
// 	}

// 	public static isGem(id:number) :boolean{
// 			return id >= 4250000 && id <= 4251402;
// 	}

// 	public static isOtherGem(id:number) :boolean{
// 			switch (id) {
// 					case 4001174:
// 					case 4001175:
// 					case 4001176:
...
// 	public static isCustomQuest(id:number) :boolean{
// 			return id > 99999;
// 	}

// 	public static getTaxAmount(meso:number):number {
// 			if (meso >= 100000000) {
...
// 	public static EntrustedStoreTax(meso:number):number {
// 			if (meso >= 100000000) {
// 					return Math.round(0.03 * meso);
// 			} else if (meso >= 25000000) {
...
// 	// public static getAttackDelay(id:number, skill:Skill):number {
...
// 	public static gachaponRareItem(id:number) {
// 			switch (id) {
...
// 			return 0;
// 	}
// 	public  static  goldrewards:number[] = [
// 			2049400, 1,
// 			2049401, 2,
// 			2049301, 2,
...
// 			2530000, 5,
// 			2531000, 5]; // Gold Box
// 	public static  silverrewards:number[] = [
// 			2049401, 2,
// 			2049301, 2,
...
// 	public  static peanuts:number[] = [2430091, 200, 2430092, 200, 2430093, 200, 2430101, 200, 2430102, 200, 2430136, 200, 2430149, 200,//mounts 
// 			2340000, 1, //rares
...
// 			2028062, 5,
// 			2028061, 5];
// 	public eventCommonReward:number[] =[
// 			0, 10,
// 			1, 10,
// 			4, 5,
// 			5060004, 25,
...
// 			5490001, 6
// 	];
// 	public static eventUncommonReward:number[]=[
// 			1, 4,
// 			2, 8,
// 			3, 8,
// 			2022179, 5,
...
// 			2049211, 1
// 	];
// 	public static eventRareReward :number[] =[
...
// 			2430148, 1,
// 			2430137, 1,];
// 	public static eventSuperReward:number[]=[
// 			2022121, 10,
// 			4031307, 50,
// 			3010127, 10,
// 			3010128, 10,
...
// 			1132085, 1
// 	];
// 	public static tenPercent:number[] =[
// 			//10% scrolls
// 			2040002,
// 			2040005,
// 			2040026,
...
// 			2048005
// 	];
// 	public static fishingReward:number[]= [
// 			0, 100, // Meso
// 			1, 100, // EXP
...
// 	public static isReverseItem(itemId:number) :boolean{
// 			switch (itemId) {
// 					case 1002790:
// 					case 1002791:
// 					case 1002792:
...
// 	public static isTimelessItem(itemId:number) :boolean{
// 			switch (itemId) {
...
// 	public static isFriendshipRing(itemId:number) :boolean{
// 			switch (itemId) {
// 					case 1112800:
// 					case 1112801:
// 					case 1112802:
...
// 	public static isCrushRing(itemId:number) :boolean{
// 			switch (itemId) {
// 					case 1112001:
// 					case 1112002:
// 					case 1112003:
...
// 			return false;
// 	}
// 	public static[] Equipments_Bonus = {1122017:number};

// 	public static Equipment_Bonus_EXP(const itemid) { // TODO : Add Time for more exp increa:numberse
// 			switch (itemid) {
// 					case 1122017:
// 							return 10;
// 			}
// 			return 0;
// 	}
// 	public static[] blockedMaps = {180000001, 180000002, 109050000, 280030000, 240060200, 280090000, 280030001, 240060201, 950101100, 950101010:number};
// 	//If you can think of more maps that could be exploitable via npc,block nao pliz!

// 	public static getExpForLevel(int i, itemId:number):number {
...
// 	public static getMaxLevel(itemId:number):number {
...
// 	public static getStatChance():number {
// 			return 25;
// 	}

// 	public static MonsterStatus getStatFromWeapon(const itemid) {
// 			switch (itemid) {
// 					case 1302109:
// 					case 1312041:
// 					case 1322067:
...
// 	public static getXForStat(MonsterStatus stat):number {
// 			switch (stat) {
// 					case BLIND:
// 							return -70;
// 					case SPEED:
...
// 	public static getSkillForStat(MonsterStatus stat):number {
// 			switch (stat) {
// 					case BLIND:
// 							return 1111003;
// 					case SPEED:
...
// 	public static getSkillBook(job:number):number {
...
// 	public static getSkillBook(job:number, level:number):number {
// 			if (job >= 2210 && job <= 2218) {
// 					return job - 2209;
// 			}
// 			switch (job) {
...
// 	public static getSkillBookForSkill(skillid:number):number {
// 			return getSkillBook(skillid / 10000);
// 	}

// 	public static getLinkedMountItem(const sourceid):number {
...
// 	public static getMountItem(const sourceid, final MapleCharacter chr):number {
// 			switch (sourceid) {
// 					case 5221006:
// 							return 1932000;
// 					case 33001001: //temp.
...
// 							} else {
// 									const link = getLinkedMountItem(sourceid);
// 									if (link > 0) {
// 											if (link < 10000) {
...
// 	public static isKatara(itemId:number) :boolean{
// 			return itemId / 10000 == 134;
// 	}

// 	public static isDagger(itemId:number) :boolean{
// 			return itemId / 10000 == 133;
// 	}

// 	public static isApplicableSkill(int skil) :boolean{
...
// 	public static isApplicableSkill_(int skil) { //not applicable to saving but is more of temporar:booleany
...
// 	public static isTablet(itemId:number) :boolean{
// 			return itemId / 1000 == 2047;
// 	}

// 	public static isGeneralScroll(itemId:number) :boolean{
// 			return itemId / 1000 == 2046;
// 	}

// 	public static getSuccessTablet(const scrollId, level:number):number {
...
// 	public static getCurseTablet(const scrollId, level:number):number {
// 			if (scrollId % 1000 / 100 == 2) { //2047_2_00 = armor, 2047_3_00 = accessory
// 					switch (level) {
// 							case 0:
// 									return 10;
...
// 	public static isAccessory(itemId:number) :boolean{
// 			return (itemId >= 1010000 && itemId < 1040000) || (itemId >= 1122000 && itemId < 1153000) || (itemId >= 1112000 && itemId < 1113000);
// 	}

// 	public static potentialIDFits(const potentialID, const newstate, const i) :boolean{
...
// 	public static optionTypeFits(const optionType, itemId:number) :boolean{
// 			switch (optionType) {
// 					case 10: //weapon
...
// 	public static final boolean isMountItemAvailable(const mountid, jobid:number) {
...
// 	public static isMechanicItem(itemId:number) :boolean{
// 			return itemId >= 1610000 && itemId < 1660000;
// 	}

// 	public static isEvanDragonItem(itemId:number) :boolean{
// 			return itemId >= 1940000 && itemId < 1980000; //194 = mask, 195 = pendant, 196 = wings, 197 = tail
// 	}

// 	public static canScroll(itemId:number) :boolean{
// 			return itemId / 100000 != 19 && itemId / 100000 != 16; //no mech/taming/dragon
// 	}

// 	public static canHammer(itemId:number) :boolean{
// 			switch (itemId) {
// 					case 1122000:
...
// 			return true;
// 	}
// 	public static[] owlItems = new int[:number]{
// 			1082002, // work gloves
// 			2070005,
// 			2070006,
// 			1092008,
...
// 	public static getMasterySkill(job:number):number {
...
// 	public static getExpRate_Below10(job:number):number {
// 			return 1;
...
// 	public static getExpRate_Quest(level:number):number {
// 			return 1;
// 			//return (level >= 30 ? (level >= 70 ? (level >= 120 ? 10 : 5) : 2) : 1);
// 	}

// 	public static String getCashBlockedMsg(id:number) {
// 			return "현재 캐시샵에서 구매 불가능한 아이템입니다.";
// 	}

// 	public static getCustomReactItem(const rid, const original):number {
...
// 	public static getJobNumber(jobz:number):number {
// 			job:number = (jobz % 1000);
...
// 	public static isBeginnerJob(job:number) :boolean{
// 			return job == 0 || job == 1 || job == 1000 || job == 2000 || job == 2001 || job == 3000 || job == 3001 || job == 2002;
// 	}

// 	public static isForceRespawn(int mapid) :boolean{
// 			switch (mapid) {
...
// 	public static getFishingTime(boolean vip, boolean gm):number {
// 			return gm ? 1000 : (vip ? 30000 : 60000);
// 	}

// 	public static getCustomSpawnID(int summoner, int def):number {
// 			switch (summoner) {
// 					case 9400589:
// 					case 9400748: //MV
// 							return 9400706; //jr
...
// 	public static canForfeit(int questid) :boolean{
// 			switch (questid) {
// 					case 20000:
// 					case 20010:
...
// 			const maxX = Math.max(Math.abs(lt == null ? 0 : lt.x), Math.abs(rb == null ? 0 : rb.x));
// 			const maxY = Math.max(Math.abs(lt == null ? 0 : lt.y), Math.abs(rb == null ? 0 : rb.y));
...
// 	public static getLowestPrice(itemId:number):number {
// 			switch (itemId) {
// 					case 2340000: //ws
// 					case 2531000:
// 					case 2530000:
...
// 	public static isNoDelaySkill(int skillId) :boolean{
...
// 	public static isNoSpawn(int mapID) :boolean{
...
// 	public static getExpRate(job:number, int def):number {
// 			return def;
// 	}

// 	public static getModifier(itemId:number, int up):number {
// 			if (up <= 0) {
// 					return 0;
// 			}
// 			switch (itemId) {
...
// 	public static short getSlotMax(itemId:number) {
// 			switch (itemId) {
// 					case 4030003:
// 					case 4030004:
// 					case 4030005:
...
// 	public static isDropRestricted(itemId:number) :boolean{
// 			return itemId == 3012000 || itemId == 4030004 || itemId == 1052098 || itemId == 1052202 || itemId == 3010000;
// 	}

// 	public static isPickupRestricted(itemId:number) :boolean{
...
// 	public static short getStat(itemId:number, int def) {
// 			switch (itemId) {
// 					case 1002419:
// 							return 5;
// 					case 1002959:
...
// 	public static short getHpMp(itemId:number, int def) {
// 			switch (itemId) {
// 					case 1122121:
// 							return 500;
// 					case 1142002:
...
// 	public static short getATK(itemId:number, int def) {
// 			switch (itemId) {
// 					case 1122121:
// 							return 3;
// 					case 1002959:
...
// 	public static short getDEF(itemId:number, int def) {
// 			switch (itemId) {
// 					case 1122121:
// 							return 250;
// 					case 1002959:
...
// 	public static isDojo(int mapId) :boolean{
// 			return mapId >= 925020100 && mapId <= 925023814;
// 	}

// 	public static getPartyPlayHP(int mobID):number {
// 			switch (mobID) {
// 					case 4250000:
// 							return 836000;
// 					case 4250001:
...
// 	public static getPartyPlayEXP(int mobID):number {
// 			switch (mobID) {
// 					case 4250000:
// 							return 5770;
// 					case 4250001:
...
// 	public static getPartyPlay(int mapId):number {
// 			switch (mapId) {
// 					case 300010000:
// 					case 300010100:
// 					case 300010200:
...
// 	public static getPartyPlay(int mapId, int def):number {
...
// 	public static isHyperTeleMap(int mapId) :boolean{
// 			for (int i : hyperTele) {
// 					if (i == mapId) {
// 							return true;
// 					}
...
// 	public static getCurrentDate_NoTime():number {
// 			final String time = FileoutputUtil.CurrentReadable_Time();
...
// 	public static isFishingMap(int mapid) :boolean{
// 			return mapid == 749050500 || mapid == 749050501 || mapid == 749050502 || mapid == 970020000 || mapid == 970020005;
// 	}

// 	public static getRewardPot(int itemid, int closeness):number {
// 			switch (itemid) {
// 					case 2440000:
// 							switch (closeness / 10) {
// 									case 0:
...
// 	public static getExpModByLevel(int level):number {
...
// 	public static isTeamMap(const mapid) :boolean{
...
// 	public static getStatDice(int stat):number {
// 			switch (stat) {
// 					case 2:
// 							return 30;
// 					case 3:
...
// 	public static getDiceStat(int buffid, int stat):number {
...
// 			return 0;
// 	}
// 	public static const[] publicNpcIds = {9270035, 9070004, 9010022, 9071003, 9000087, 9000088, 9010000, 9000085, 9000018, 9000000};
...
// 	public static const OMOK_SCORE = 122200;
// 	public static const MATCH_SCORE = 122210;
// 	public static const HP_ITEM = 122221;
// 	public static const MP_ITEM = 122223;
// 	public static const JAIL_TIME = 123455;
// 	public static const JAIL_QUEST = 123456;
// 	public static const REPORT_QUEST = 123457;
// 	public static const ULT_EXPLORER = 111111;
// 	//codex = -55 slot
...
// 	public static const POKEMON_WINS = 122400;
// 	public static const ENERGY_DRINK = 122500;
// 	public static const HARVEST_TIME = 122501;
// 	public static const PENDANT_SLOT = 122700;
// 	public static const CURRENT_SET = 122800;
// 	public static const BOSS_PQ = 150001;
// 	public static const JAGUAR = 111112;
// 	public static const DOJO = 150100;
// 	public static const DOJO_RECORD = 150101;
// 	public static const PARTY_REQUEST = 122900;
// 	public static const PARTY_INVITE = 122901;
// 	public static const QUICK_SLOT = 123000;
// 	private static const[] prmaps = {990000500, 209000001, 209000002, 209000003, 209000004, 209000005, 209000006, 209000007, 209000008, 209000009, 209000010, 209000011, 209000012, 209000013, 209000014, 209000015};

// 	public static isPickupRestrictedMap(int map) :boolean{
// 			for (int i : prmaps) {
// 					if (i == map) {
// 							return true;
// 					}
...
// 	public static getEquipQuality(final Equip equip):number {
